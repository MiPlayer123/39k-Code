  ...
  void validateIndexes(size_t row, size_t col) const {
    if (row < 0 or row >= mRows)
      throw invalid_argument(
          "Invalid row index: should be between 0 and " + to_string(mRows - 1));
    if (col < 0 or col >= mCols)
      throw invalid_argument(
          "Invalid column index: should be between 0 and " + to_string(mCols - 1));
  }
  ...
  friend ostream &operator<<(ostream &os, const Matrix &matrix) {
    ...
    for (int i = 0; i < matrix.mRows; i++) {
      for (int j = 0; j < matrix.mCols; j++) {
        ...
        os << std::left << setw(numWidth) << setfill(fill) << to_string(matrix(i, j));
      }
      os << endl;
    }
